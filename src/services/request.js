...
const cachedSave = (response, hashcode) => {
    ...
    if (contentType && contentType.match(/application\/x-www-form-urlencoded/i)) {
...
export default function request(url, option) {
    ...
    const apiUrl = getAPIUrl(url, option?.type);
    /**
     * Produce fingerprints based on url and parameters
     * Maybe url has the same parameters
     */
    const fingerprint = apiUrl + (options.body ? JSON.stringify(options.body) : '');
    const hashcode = hash
        .sha256()
        .update(fingerprint)
        .digest('hex');
    ...
    if (
        ...
        if (!(newOptions.body instanceof FormData)) {
            ...
            newOptions.body = obj2str(newOptions.body);
        } else {
